...
<main id="skip-content">
      ...
      <?php  while ( $query->have_posts() ) : $query->the_post(); ?>
        <div class="slider-box">
          ...
          <div class="slider-inner-box">
            <h1><?php the_title(); ?></h1>
            <div class="my-4">
              <a href="<?php the_permalink(); ?>"><?php esc_html_e('VIEW MORE','boutique-garment-store'); ?></a>
            </div>
          </div>
        </div>
      <?php $i++; endwhile;
        ...
        <div class="row serv-bg-box">
          <?php
            ...
            if($digital_storefront_catData){
              $digital_storefront_page_query = new WP_Query(array( 'category_name' => esc_html($digital_storefront_catData,'boutique-garment-store')));
  ...
  <section id="new-products" class="py-5">
    <div class="container">
      <?php if(get_theme_mod('boutique_garment_store_new_product_title') != ''){ ?>
        <h3 class="text-center"><?php echo esc_html(get_theme_mod('boutique_garment_store_new_product_title','')); ?></h3>
        <hr>
      <?php }?>
      <?php if(get_theme_mod('boutique_garment_store_new_product_text') != ''){ ?>
        <p class="text-center"><?php echo esc_html(get_theme_mod('boutique_garment_store_new_product_text','')); ?></p>
      <?php }?>
      <div class="row mt-4">
        <?php
        if ( class_exists( 'WooCommerce' ) ) {
          $args = array( 
            'post_type' => 'product',
            'posts_per_page' => get_theme_mod('boutique_garment_store_new_product_number'),
            'product_cat' => get_theme_mod('boutique_garment_store_new_product_category'),
            'order' => 'ASC'
          );
